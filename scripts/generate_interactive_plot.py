...
class InteractivePlotGenerator:
    ...
    def _agregar_nodos(self, fig: go.Figure):
        ...
        for tipo, nodos_grupo in nodos_por_tipo.items():
            ...
            # Using 'bottom center' here, JavaScript will override to 'auto' for better positioning
    ...
    <section class="hero" id="hero">
        <div class="hero-content">
            ...
            <p class="description">
                ...
                Una herramienta pedagógica inspirada en la investigación de Ha-Joon Chang (Economics: The User's Guide, 2015).
            </p>
...
{guide_section}
    ...
    <section class="container" id="visualization">
        <div class="visualization-header">
            ...
            <!-- DOUBLE DROPDOWN SELECTOR -->
            <div style="margin-top: 25px; text-align: center;">
                <div style="display: inline-block; margin-right: 30px; margin-bottom: 15px;">
                    <label for="preset-dropdown" style="font-weight: 600; color: #2c3e50; margin-right: 10px; font-size: 0.95em;">
                        Perspectiva de Peso:
                    </label>
                    <select id="preset-dropdown" style="padding: 10px 20px; font-size: 1em; border: 2px solid #667eea; border-radius: 8px; background: white; color: #667eea; cursor: pointer; font-weight: 600; min-width: 200px;">
                        <option value="base" selected>Base (Original)</option>
                        <option value="balanced">Balanceado</option>
                        <option value="state-emphasis">Enfoque Estado</option>
                        <option value="equity-emphasis">Enfoque Equidad</option>
                        <option value="market-emphasis">Enfoque Mercado</option>
                        <option value="growth-emphasis">Enfoque Crecimiento</option>
                        <option value="historical-emphasis">Enfoque Historico</option>
                        <option value="pragmatic-emphasis">Enfoque Pragmatico</option>
                    </select>
                </div>

                <div style="display: inline-block; margin-bottom: 15px;">
                    <label for="normalization-dropdown" style="font-weight: 600; color: #2c3e50; margin-right: 10px; font-size: 0.95em;">
                        Metodo de Normalizacion:
                    </label>
                    <select id="normalization-dropdown" style="padding: 10px 20px; font-size: 1em; border: 2px solid #764ba2; border-radius: 8px; background: white; color: #764ba2; cursor: pointer; font-weight: 600; min-width: 220px;">
                        <option value="percentile" selected>Percentil (Distribucion Uniforme)</option>
                        <option value="zscore">Puntaje-Z (Centrado Estadistico)</option>
                        <option value="minmax">Min-Max (Rango Completo)</option>
                        <option value="none">Crudo (Sin Normalizacion)</option>
                    </select>
                </div>
            </div>
        </div>
...
{leyenda_html}
    ...
    <footer class="footer">
        ...
        <p style="margin-top: 10px; font-size: 0.85em;">
            Proyecto: Mapa de Escuelas Políticas Económicas • 2025
        </p>
        <p style="margin-top: 10px; font-size: 0.85em;">
            <strong>Autor:</strong> William Camilo Artunduaga Viana (Propio de Autor)
        </p>
        <p style="margin-top: 5px; font-size: 0.85em;">
            📜 Licencia: <a href="https://creativecommons.org/licenses/by-sa/4.0/" target="_blank" style="color: #667eea; text-decoration: underline;">Creative Commons BY-SA 4.0</a>
            • Código abierto para mejoras comunitarias
        </p>
    </footer>
...
{javascript}
    ...
    <style>
        * {
        ...
        .guide-section { background: white; padding: 50px 15px; }
        ...
        .container { max-width: 100%; margin: 0 auto; background: white; padding: 20px 10px; }
        .visualization-header { text-align: center; margin-bottom: 20px; }
        .visualization-header h2 { font-size: 2.2em; color: #2c3e50; margin-bottom: 10px; }

        .main-content { max-width: 1800px; margin: 0 auto; }
        .plot-container { width: 100%; margin-bottom: 15px; }
        ...
        @media (max-width: 1200px) {
            .pedagogical-legend { max-width: 100%; }
        }
        ...
        <div class="guide-content">
            ...
            <div class="guide-grid">
                ...
                <div class="guide-card">
                    ...
                    <ul>
                        <li><strong>Q1:</strong> Estado Fuerte + Equidad Y Sostenibilidad</li>
                        <li><strong>Q2:</strong> Estado Débil + Equidad Y Sostenibilidad</li>
                        <li><strong>Q3:</strong> Estado Fuerte + Crecimiento y Productividad</li>
                        <li><strong>Q4:</strong> Estado Débil + Crecimiento y Productividad</li>
                    </ul>
                </div>
                ...
                <div class="guide-card">
                    <h3>📚 Metodología de Visualización</h3>
                    <p>Basado en el capitulo 4 (Economics: The User's Guide), de <strong>Ha-Joon Chang</strong>:</p>
                    <ul>
...
"""
    ...
    def _generar_variant_selector(self) -> str:
        """Genera el doble selector de variantes (peso + normalizacion)."""
        # No hay seccion de selector separada - se integra en la seccion de visualizacion
        return ""

    def _generar_javascript(self) -> str:
        """Genera el JavaScript para interactividad y carga dinamica de variantes."""
        return """
    <script>
        // Smooth scroll
        ...
        // Global data stores
        let baseData = null;  // Source of truth from escuelas.json
        let currentVariantData = null;  // Current variant positions
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            // Load base data (source of truth)
            try {
                ...
                console.log('Base data loaded from escuelas.json:', baseData);
            } catch (error) {
                console.error('CRITICAL: Failed to load base data:', error);
                showNotification('Error critico: No se pudo cargar escuelas.json', 'error');
                return;
            }
            ...
            presetDropdown.addEventListener('change', loadCurrentVariant);
            normalizationDropdown.addEventListener('change', loadCurrentVariant);
        ...
        async function loadCurrentVariant() {
            ...
            const normalization = document.getElementById('normalization-dropdown').value;
            const variantName = preset + '-' + normalization;

            console.log('Loading variant: ' + variantName);
            showNotification('Cargando variante: <strong>' + variantName + '</strong>', 'info');

            try {
                // Fetch variant position data
                ...
                // Merge variant positions with base data
                const mergedData = mergeVariantWithBase(variantData, baseData);

                // Update the Plotly chart
                updatePlotlyChart(mergedData);

                showNotification('Variante cargada: <strong>' + variantName + '</strong>', 'success');
            } catch (error) {
                console.error('Error loading variant:', error);

                let errorMsg = 'Error al cargar variante ' + variantName + ': ' + error.message;
                if (error.message.includes('Failed to fetch') || window.location.protocol === 'file:') {
                    errorMsg = 'CORS Error: Debes abrir desde servidor HTTP. Usa: python -m http.server 8000';
                }
        ...
        function mergeVariantWithBase(variantData, baseData) {
            // Merges variant position data with base descriptive data.
            // Variant contains only positions, base contains descriptions, colors, transitions.
            const merged = {
                metadata: variantData.metadata || {},
                nodos: [],
                transiciones: baseData.transiciones || [],  // Transitions from base
                mapeo_visual: baseData.mapeo_visual || {}
            };

            // Create lookup map for base nodes
            const baseNodesMap = {};
            (baseData.nodos || []).forEach(node => {
                baseNodesMap[node.id] = node;
            });

            // Merge each variant node with its base data
            (variantData.nodos || []).forEach(variantNode => {
                const baseNode = baseNodesMap[variantNode.id];

                if (!baseNode) {
                    console.warn('Base data missing for node: ' + variantNode.id);
                    merged.nodos.push(variantNode);  // Use variant data as fallback
                    return;
                }

                // Merge: take position from variant, everything else from base
                const mergedNode = {
                    ...baseNode,  // All base data (descriptions, color, tipo, etc.)
                    posicion: variantNode.posicion  // Position from variant
                };

                merged.nodos.push(mergedNode);
            });
        ...
        function updatePlotlyChart(data) {
            ...
            nodos.forEach((node, idx) => {
                // Use node color if available, otherwise use Klein schema
                if (node.color) {
                    nodeColors[node.id] = node.color;
                } else if (node.tipo === 'tradicion') {
            ...
            Object.entries(nodesByType).forEach(([tipo, nodes]) => {
                ...
                // Build hover templates with full descriptions
                const hoverTexts = nodes.map(node => {
                    let hoverText = '<b>' + node.nombre + '</b><br>';
                    hoverText += '<i>' + (node.descripcion_corta || '') + '</i><br><br>';

                    // Add descriptor information if available
                    if (node.descriptores) {
                        const desc = node.descriptores;
                        hoverText += '<b>Descriptores:</b><br>';
                        if (desc.rol_estado !== undefined) hoverText += '• Rol Estado: ' + desc.rol_estado.toFixed(2) + '<br>';
                        if (desc.prioridad_equidad !== undefined) hoverText += '• Equidad: ' + desc.prioridad_equidad.toFixed(2) + '<br>';
                        if (desc.enfoque_crecimiento !== undefined) hoverText += '• Crecimiento: ' + desc.enfoque_crecimiento.toFixed(2) + '<br>';
                    }

                    hoverText += '<br><b>Posicion:</b> (' + node.posicion.x.toFixed(2) + ', ' + node.posicion.y.toFixed(2) + ')';
                    return hoverText;
                });
                ...
                const trace = {
                    ...
                    hovertemplate: hoverTexts.map(ht => ht + '<extra></extra>')
                };
